...
class GradientReconstructor_2():
    ...
    def reconstruct(self, input_data, labels, layer_grads, img_shape=(3, 32, 32), dryrun=False, eval=True, tol=None):
        ...
        try:
            for trial in range(self.config['restarts']):
                grad_sims = self._run_trial(x[trial], input_data, labels, layer_grads, dryrun=dryrun)
                # Finalize
    ...
    def _run_trial(self, x_trial, input_data, labels, layer_grads, dryrun=False):
        ...
        try:
            ...
            for iteration in range(max_iterations):
                ...
                grad_sim = self._grad_sim(optimizer, x_trial, input_data, labels, layer_grads)
                grad_sims.append(grad_sim)
    ...
    def _grad_sim(self, optimizer, x_trial, input_gradient, label, layer_grads):
        optimizer.zero_grad()
        self.model.zero_grad()
        loss = self.loss_fn(self.model(x_trial), label)
        ...
        grad_sim = []
        for ig, g in zip(layer_grads, grads):
            # grad_sim.append([cal_dis(dw_a, dw_b, metric='cos') for dw_a, dw_b in zip(g, ig)] / len(ig))
            grad_sim.append(cal_dis(g, ig, metric='cos'))
...
class FedAvgReconstructor(GradientReconstructor_2):
